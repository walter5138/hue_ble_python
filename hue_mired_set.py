...
while True:
    ...
    if x == 'k':
        hl_1.mired_set(y)
        time.sleep(hl_1.transitiontime_get())   # if transitiontime is short values needs time to settle.
        ...
        time.sleep(hl_1.transitiontime_get())   # if transitiontime is short values needs time to settle.
        print("Lamp %s got mired %s in %s seconds." % (colored(hl_1.name, 'yellow'), colored(hl_1.mired_get(), 'green'),  colored(hl_1.transitiontime_get(), 'green')))
        time.sleep(hl_2.transitiontime_get())   # if transitiontime is short values needs time to settle.
        ...
        time.sleep(hl_2.transitiontime_get())   # if transitiontime is short values needs time to settle.
        print("Lamp %s got mired %s in %s seconds." % (colored(hl_2.name, 'yellow'), colored(hl_2.mired_get(), 'green'),  colored(hl_2.transitiontime_get(), 'green')))
        time.sleep(hl_3.transitiontime_get())   # if transitiontime is short values needs time to settle.
